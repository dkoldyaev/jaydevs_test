import { DefaultChangesCosts } from './consts';
...
export const buildDistancesMatrix = (
  ...
  for (let i = 0; i < word1.length; i++) {
    for (let j = 0; j < word2.length; j++) {
      ...
      dp[i][j] = Math.min(
        insert + (costInsert || DefaultChangesCosts[ChangesTypes.INSERT]),
        del + (costDelete || DefaultChangesCosts[ChangesTypes.DELETE]),
        replace + (word1[i] === word2[j]
          ? 0
          : (costReplace || DefaultChangesCosts[ChangesTypes.REPLACE])
        )
      );
    }
  }
...
export const buildChains = (
  ...
  const curWord = Array.from(word2);
